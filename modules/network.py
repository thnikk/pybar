...
def get_devices():
    ...
    try:
        res = run(['nmcli', '-t', '-f', 'DEVICE,TYPE,STATE,CONNECTION',
                  'd'], check=True, capture_output=True).stdout.decode('utf-8')
        devices = []
...
def get_wifi_networks():
    ...
    try:
        ...
        remembered = get_remembered_ssids()
        networks = []
        for line in res.splitlines():
            parts = line.rsplit(':', 4)
...
def fetch_data(config):
    ...
    return {
        ...
        "config": config  # Pass config for UI updates
    }
...
def create_widget(bar, config):
    ...
    module.module_name = config.get('name', 'network')
    return module
...
def wifi_action(
        ssid, password=None, disconnect=False, forget=False,
        button=None, module_name=None, config=None):
    ...
    def run_nmcli():
        try:
            ...
            if module_name and config:
                new_data = fetch_data(config)
                c.state_manager.update(module_name, new_data)
...
def toggle_wifi_details(btn, details_box, indicator):
...
def build_popover(module, data):
    ...
    module.popover_widgets = {}
    # Module name is needed for real-time updates.
    # We can try to infer it from the state manager or pass it.
    # In pybar, the module object doesn't explicitly store its name,
    # but we can assume "network" or get it from the state manager's
    # subscribers.
    # A better way is to store it during create_widget if possible.
    # For now, let's assume "network" as default or use a known one.
    module_name = getattr(module, 'module_name', 'network')
    ...
    if data.get('wifi_networks'):
        ...
        for i, net in enumerate(data['wifi_networks']):
            ...
            signal_label = c.label(f"{net['SIGNAL']}%", style='gray', ha='end')
            ...
            if net['IN-USE'] or net['REMEMBERED']:
                ...
                if net['IN-USE']:
                    action_btn = c.button("Disconnect", style='red', ha='fill')
                    action_btn.connect(
                        'clicked', lambda b, s=net['SSID']: wifi_action(
                            s, disconnect=True, button=b,
                            module_name=module_name, config=config))
                else:
                    action_btn = c.button("Connect", style='blue', ha='fill')
                    action_btn.connect(
                        'clicked', lambda b, s=net['SSID']: wifi_action(
                            s, button=b, module_name=module_name,
                            config=config))
                ...
                forget_btn.connect(
                    'clicked', lambda b, s=net['SSID']: wifi_action(
                        s, forget=True, button=b, module_name=module_name,
                        config=config))
                ...
                def on_connect(b, s=net['SSID'], e=pass_entry):
                    wifi_action(s, e.get_text(), button=b,
                                module_name=module_name, config=config)

                connect_btn.connect('clicked', on_connect)
                pass_entry.connect(
                    'activate', lambda e: on_connect(connect_btn))
            ...
            ssid_btn.connect('clicked', toggle_wifi_details,
