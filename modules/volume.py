...
def create_widget(bar, config):
    ...
    scroll_controller.connect('scroll', lambda c, dx,
                              dy: handle_scroll(module, dx, dy))
    module.add_controller(scroll_controller)

    # Add right-click for mute
    click_controller = Gtk.GestureClick.new()
    click_controller.set_button(3)  # Right click
    click_controller.connect('released', lambda c, n,
                             x, y: toggle_default_mute())
    ...
    middle_click_controller.connect(
        'released', lambda c, n, x, y: cycle_output_device(
...
def build_device_row(section, device):
    ...
    mute_switch.connect('state-set', lambda s,
                        state: toggle_mute(section, device['index'], state))
    ...
    slider.connect('value-changed', lambda s: set_dev_volume(section,
                   device['index'], s.get_value()))
    if device.get('mute'):
