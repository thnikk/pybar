...
def fetch_data(config):
    ...
    return {
        "text": config.get('format', '{}').replace('{}', data['state'].split('.')[0]),
...
def update_ui(module, data):
    ...
    if not module.get_active():
        ...
        if module.graph:
            module.graph.update_data(data.get('history', []), data.get('state'))
        if module.duration_label:
            module.duration_label.set_text(f"{data.get('duration', 0):.0f}s ago")
...
def build_popover(module, data):
    ...
    if data.get('history'):
        ...
        module.duration_label = c.label(duration_str, style='gray', ha='start', he=True)
