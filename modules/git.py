    ...
    try:
        ...
        for line in res.splitlines():
            if re.match('^commit', line):
                ...
                output[chash] = {"author": None,
                                 "date": None, "msg": None, "files": []}
...
def toggle_details(btn, details_box, indicator):
...
def build_popover(data):
    ...
    for chash, info in data['commits'].items():
        ...
        if len(info['files']) > 10:
            file_box.append(
                c.label(f"...and {len(info['files'])-10} more", style='gray'))
        ...
        btn.connect('clicked', toggle_details, details_box, indicator)
