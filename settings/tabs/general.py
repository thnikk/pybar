...
class GeneralTab(Gtk.Box):
    ...
    def __init__(self, config, on_change):
        ...
        self.floating_group.set_description("Configure floating bar appearance")
        ...
        for key, schema_field in GLOBAL_SCHEMA.items():
            ...
            if schema_field.get("type") == FieldType.CHOICE:
                ...
                self.size_group.add_widget(dropdown)
                ...
                class SimpleEditor:
                    ...
                    def get_value(self):
                        idx = self.dropdown.get_selected()
                        return (
                            self.choices[idx] if 0 <= idx < len(self.choices) else None
                        )
                ...
                reset_btn.connect(
                    "clicked",
                    lambda _, k=key, e=editor, s=schema_field: e.set_value(
                        s.get("default")
                    ),
                )
                row.add_suffix(reset_btn)
